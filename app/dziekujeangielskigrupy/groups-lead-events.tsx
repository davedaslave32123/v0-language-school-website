...
declare global {
  interface Window {
    ...
    __englishGroupsLeadEventsSent?: boolean
  }
}
 ...
 */
export function GroupsLeadEvents() {
  useEffect(() => {
    let cancelled = false
    let pollTimer: ReturnType<typeof setInterval> | null = null

    const fireEvents = () => {
      if (cancelled || window.__englishGroupsLeadEventsSent) return

      window.dataLayer = window.dataLayer || []
      window.dataLayer.push({
        event: "leadFormSubmit",
        formSource: "angielskigrupy",
      })
      ...
      pollTimer = setInterval(() => {
        attempts += 1
        if (window.fbq) {
          window.fbq("track", "Lead", { content_name: "AngielskiGrupy" })
      ...
      window.__englishGroupsLeadEventsSent = true
    }
